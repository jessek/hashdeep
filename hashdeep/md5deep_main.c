...
/**
 * the md5deep original version put the hash setup inside the state variable.
 * hashdeep allows multiple hashing, so for md5deep we set it up in a single
 */
...
int md5deep_main(int argc, char **argv) 
{
  ...
  if (md5deep_process_command_line(s,argc,argv))
  {
  ...
  if ((s->mode & mode_match) || (s->mode & mode_match_neg))
    s->return_value = md5deep_finalize_matching(s);
